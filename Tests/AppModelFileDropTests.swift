...
struct AppModelFileDropTests {
    ...
    static func main() {
        ...
        // MARK: - Compact size follows the hardware notch

        do {
            let model = makeModel()
            expect(model.mode == .compact, "model starts compact for sizing checks")
            expect(
                model.currentSize == NSSize(width: 310, height: 32),
                "fallback compact size without notch metrics (200 notch + 36+36 wings + 19+19 padding)"
            )

            model.updateNotchMetrics(PhysicalNotchMetrics(width: 220, height: 34))
            expect(
                model.currentSize == NSSize(width: 330, height: 34),
                "compact size tracks hardware notch metrics"
            )

            model.showMessage("Wing test message")
            expect(
                model.currentSize == NSSize(width: 498, height: 34),
                "transient message widens compact wings (220 + 120+120 + 19+19)"
            )
        }
