...
struct SmokeTests {
    ...
    static func main() throws {
        ...
        let hardwareNotch = NotchGeometry.metrics(
            screenWidth: 1512,
        ...
        expect(
            hardwareNotch == PhysicalNotchMetrics(width: 200, height: 32),
            "hardware notch metrics derived from screen values"
        )
        expect(
            ...
            "plain displays report no hardware notch"
        )
        let musicWings = NotchGeometry.wingWidths(
            presentation: .music,
            configuredContent: .music,
            isTimerActive: false,
            hasTransientMessage: false
        )
        expect(musicWings == WingWidths(left: 36, right: 36), "music wings are icon sized")
        expect(
            ...
            "active timer widens the right wing for the countdown"
        )
        expect(
            NotchGeometry.wingWidths(
                ...
                isTimerActive: false,
                hasTransientMessage: false
            ) == WingWidths(left: 36, right: 64),
            "download uses a text wing for the progress percent"
        )
        expect(
            NotchGeometry.wingWidths(
            ...
            "calendar uses a text wing for the next event time"
        )
        expect(
            NotchGeometry.wingWidths(
                presentation: .configured,
                configuredContent: .todo,
                isTimerActive: false,
                hasTransientMessage: false
            ) == WingWidths(left: 36, right: 36),
            "todo wings are icon sized"
        )
        expect(
            NotchGeometry.wingWidths(
                presentation: .music,
            ...
            ) == WingWidths(left: 120, right: 120),
            "transient message widens both wings"
        )
        expect(
            NotchGeometry.compactSize(
                notch: hardwareNotch,
                wings: musicWings,
                leadingPadding: 19,
                trailingPadding: 19
            ) == CGSize(width: 310, height: 32),
            "compact size hugs the notch plus wings and padding"
        )
        expect(
            NotchGeometry.compactSize(
                notch: nil,
                ...
                leadingPadding: 0,
                trailingPadding: 0
            ) == CGSize(width: 272, height: 32),
            "fallback notch sizing on plain displays"
        )
        ...
        expect(store.load().resolvedExpandedContentTopPadding == 46, "expanded content top padding persistence")
        ...
        expect(settingsWithoutAppearance.resolvedExpandedContentTopPadding == 38, "legacy expanded content top padding default includes clearance")
