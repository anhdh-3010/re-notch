...
final class AppModel: ObservableObject {
    ...
    func updateNotchMetrics(_ metrics: PhysicalNotchMetrics?) {
        guard metrics != notchMetrics else { return }
    ...
    var currentSize: NSSize {
        switch mode {
        case .compact:
            let wings = NotchGeometry.wingWidths(
                presentation: compactPresentation,
                configuredContent: settings.resolvedCompactContent,
                isTimerActive: timer.isActive,
                hasTransientMessage: transientMessage != nil
            )
    ...
    func hoverChanged(_ hovering: Bool) {
        ...
        if hovering {
            ...
            // Hover-out mirrors the Dynamic Island: retract immediately.
            collapse()
        }
    }
